...
with st.sidebar:
    ...
    # Secure Cloud Pull
    try:
        # The app looks in your secure backend for the hidden link
        cloud_url = st.secrets["CLOUD_CSV_URL"]
        
        if st.button("📥 Pull Latest Cloud Inventory", type="primary"):
            with st.spinner("Fetching from cloud..."):
                try:
                    df = pd.read_csv(cloud_url)
                    df['Length'] = df['Length'].astype(float).round(2)
                    st.session_state.inventory = df
                    st.success("✅ Synced with Cloud Database!")
                except urllib.error.URLError:
                    st.error("❌ Failed to reach the cloud file. Check the link.")
                except Exception as e:
                    st.error(f"❌ Format error: Make sure it is a valid CSV. ({e})")
        st.divider()
    except (FileNotFoundError, KeyError):
        st.info("Cloud sync not configured. See instructions to set up secrets.")
    ...
    if uploaded_file:
        ...
        df['Length'] = df['Length'].astype(float).round(2)
        st.session_state.inventory = df
        st.success("✅ Local Inventory Loaded!")
...
if not st.session_state.inventory.empty:
    ...
    st.info("Upload or Sync inventory to begin.")
